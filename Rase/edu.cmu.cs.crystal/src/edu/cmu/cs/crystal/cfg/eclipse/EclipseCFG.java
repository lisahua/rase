 ...
 * going with that.
 * 
 * Note: Since this was written someone (Thomas? Nels?) did need to transfer on
 * the control flow. He needed it at the merge point though. The subtype
 * 
 * @link{EclipseNodeFirstCFG handles that, and is currently the default control
 *                           flow graph used in Crystal.
 * 
 *                           Algorithm description: This class visits each node
 *                           of the AST. When it visits a node, it maps the
 *                           ASTNode to the CFGNode that it is creating. This
 *                           allows a parent node to pull it out later and
 *                           insert it into the graph. There are several other
 *                           data structures to help us with control flow. These
 *                           are described individually later. - In the previsit
 *                           method, we create the node and put it in the map. -
 *                           In the visit method, we prepare any data structures
 *                           that will be needed by the children. - In the
 *                           endvisit method, we put together the CFG for this
 *                           node and its children (with the exception of any
 *                           edges that were added by children through the data
 *                           structures.
 * 
 * @author ciera
 */
public class EclipseCFG extends ASTVisitor implements
	...
	public Map<ASTNode, EclipseCFGNode> getNodeMap() {
		return this.nodeMap;
	}
	...
	protected void createBooleanEdge(EclipseCFGNode source,
			EclipseCFGNode sink, boolean boolValue) {
	...
	/**
	 * Creates an edge DIRECTLY from source to the proper finally blocks, and
	 * then from the finally blocks DIRECTLY to the destination. By directly,
	 * this means that it won't call getEnd or getStart. It uses the
	 * exceptionToStopAt to determine which finally blocks to grab. It will also
	 * make the edge from the source to the finally block have this exception. A
	 * normal edge will be used when exceptionToStopAt is null
	 */
	protected void hookFinally(EclipseCFGNode source,
			ITypeBinding exceptionToStopAt, EclipseCFGNode dest) {
	...
	private EclipseCFGNode copySubgraph(EclipseCFGNode current) {
		HashMap<EclipseCFGNode, EclipseCFGNode> cloneMap = new HashMap<EclipseCFGNode, EclipseCFGNode>();
	...
	/**
	 * Modified by Na Meng to prevent infinite loop when enumerating edges
	 * 
	 * @param current
	 * @return
	 */
	private EclipseCFGNode copySubgraphRecur(EclipseCFGNode current,
			EclipseCFGNode stopNode,
			HashMap<EclipseCFGNode, EclipseCFGNode> cloneMap) {
		ASTNode node = current.getASTNode();
		EclipseCFGNode clone = cloneMap.get(current);
		if (clone != null)
			return clone;

		clone = new EclipseCFGNode(node);
		...
		if (current == stopNode || node instanceof ReturnStatement
			...
			for (EclipseCFGEdge edge : current.getOutputs()) {
				EclipseCFGEdge cloneEdge = new EclipseCFGEdge(clone,
						edge.getSink(), edge.getLabel());
				edge.getSink().addInputEdge(cloneEdge);
				clone.addOutputEdge(cloneEdge);
			}
		} else {
			for (EclipseCFGEdge edge : current.getOutputs()) {
				EclipseCFGNode cloneSink = copySubgraphRecur(edge.getSink(),
						stopNode, cloneMap);
				EclipseCFGEdge cloneEdge = new EclipseCFGEdge(clone, cloneSink,
						edge.getLabel());
	...
	public void endVisit(AssertStatement node) {
		...
		ITypeBinding binding = node.getAST().resolveWellKnownType(
				"java.lang.Throwable");
	...
	public boolean visit(MethodDeclaration node) {
		...
		exceptionMap.pushCatch(undeclExit,
				node.getAST().resolveWellKnownType("java.lang.Throwable"));
		...
		for (Name name : (List<Name>) node.thrownExceptions()) {
			...
			exceptionMap.pushCatch(implicitCatch,
	...
	public void endVisit(InfixExpression node) {
		...
		if (op.equals(Operator.CONDITIONAL_AND)
				|| op.equals(Operator.CONDITIONAL_OR)) {
	...
	public void endVisit(ClassInstanceCreation node) {
		...
		if (node.getType().resolveBinding() != null) {
			...
			constructor.setName("new " + node.getType().toString());
		}
	}
	...
	public void endVisit(MethodInvocation node) {
		...
		// normal edges
		makeListEdges(nodeMap.get(node.getExpression()),
				(List<ASTNode>) node.arguments(), invocation);
