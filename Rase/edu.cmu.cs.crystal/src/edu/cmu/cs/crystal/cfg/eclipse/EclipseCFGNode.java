...
/**
 * This class has several purposes: 1) Wrap an ASTNode 2) Be a node in a CFG,
 * with incoming and outgoing edges 3) Help us create a flat structure out of a
 * heirarchical one.
 * 
 * 1 and 2 are obvious, so we'll discuss 3. When we build a CFG, we are walking
 * a hierarchical structure (the AST) and turning it into a flat structure (the
 * CFG). When we get to a node, we must evaluate the children and then the node
 * itself. However, we might later need to plug this subgraph into another part
 * of the graph (like the else branch of an if). To do this, we need to know the
 * starting point of the subgraph. Our node can tell what it's first child is,
 * but what about it's child's child?
 * 
 * To solve this, we have a 'startNode' which keeps track of the first node. Our
 * startNode will be set to our first child's start node. This bottoms out in a
 * case where the start node is oneself.
 * 
 * We also need to know the ending node. In most cases, this is ourself.
 * However, sometimes the node breaks control flow entirely, and it can not be
 * hooked up to another node in the normal flow. To handle this, the endNode
 * might be null.
 * 
 * @author ciera
 * 
...
public class EclipseCFGNode implements ICFGNode<ASTNode> {
	...
	/**
	 * Filters the output edges by the label we are looking for. If we find a
	 * combination of BooleanLabels with anything else, we will throw an
	 * exception. We're not yet sure what this means, and we'd like to
	 * investigate this case thoroughly.
	 * 
	 * @param label
	 *            the label to search for, using .equals()
	 * @return The output edges that have the ILabel label, or the empty set if
	 *         non exist
	 */
		...
		for (EclipseCFGEdge edge : inputs) {
			if (edge.label instanceof BooleanLabel)
				...
				throw new CrystalRuntimeException(
						"Node has input labels that are booleans and others: "
								+ node);
