...
public abstract class MotherFlowAnalysis<LE> implements IFlowAnalysis<LE> {
	...
	public static final Logger log = Logger.getLogger(MotherFlowAnalysis.class
			.getName());

	/**
	 * the resulting lattices for each node in the control flow graph for the
	 * method analyzed last. Whenever results for an AST node are requested that
	 * are not in the last analyzed method, the current results are tossed and
	 * replaced with results for the method surrounding that node.
	 */
	 ...
	 * Previously computed results may still be available. <i>If</i> a monitor
	 * is set then subsequent accesses to analysis results may through a
	 * {@link java.util.concurrent.CancellationException} which, if not caught,
	 * will abort the current overall Crystal analysis job.
	 * 
	 * @param monitor
	 *            Monitor to listen for cancellation or {@link Option#none()} if
	 *            worklist runs should not be canceled.
	 */
	...
	/**
	 * Retrieves the analysis state that exists <b>before</b> analyzing the
	 * node, including <b>bottom</b>.
	 * 
	 * Before is respective to normal program flow and not the direction of the
	 * analysis.
	 * 
	 * @param node
	 *            the {@link ASTNode} of interest
	 * @return the lattice that represents the analysis state before analyzing
	 *         the node. Will be bottom if the node doesn't have a corresponding
	 *         control flow node.
	 */
	 ...
	 *            the {@link ASTNode} of interest
	 * @return the lattice that represents the analysis state before analyzing
	 *         the node. Will be <code>null</code> if the node doesn't have a
	...
	protected LE getResultsOrNullBefore(ASTNode node) {
		...
		if (cfgnodes == null || cfgnodes.isEmpty()) {
			if (log.isLoggable(Level.FINE))
				log.fine("Unable to get results before node: "
	 ...
	 * including <b>bottom</b>.
	 * 
	 * After is respective to normal program flow and not the direction of the
	 * analysis.
	 * 
	 * @param node
	 *            the {@link ASTNode} of interest
	...
	protected LE getResultsOrNullAfter(ASTNode node) {
		...
		if (cfgnodes == null || cfgnodes.isEmpty()) {
			if (log.isLoggable(Level.FINE))
				log.fine("Unable to get results after node: "
						+ getNodeDebugInfo(node));
			return null;
		} else if (cfgnodes.size() == 1) {
	...
	private String getNodeDebugInfo(ASTNode node) {
		return node.toString()
				+ " type: "
				+ ASTNode.nodeClassForType(node.getNodeType()).getName()
				+ (node.getParent() != null ? " parent type: "
						+ ASTNode.nodeClassForType(
								node.getParent().getNodeType()).getName() : "");
	}

	protected LE mergeResults(HashMap<ICFGNode<ASTNode>, LE> results,
			ASTNode node) {
		if (results.isEmpty())
		...
		for (LE r : results.values()) {
			if (result == null)
				...
				result = currentLattice.join(result, currentLattice.copy(r),
						node);
		}
		return result;
	}
	 ...
	 * @param node
	 *            the {@link ASTNode} of interest
	 * @return the lattice that represents the analysis state after analyzing
	 *         the node. Will be bottom if the node doesn't have a corresponding
	 *         control flow node.
	 */
	public IResult<LE> getLabeledResultsBefore(ASTNode node) {
		...
		return result == null ? new SingleResult<LE>(currentLattice.bottom())
				: result;
	}
	 ...
	 * @return The result of merging the given results into one, or
	 *         <code>null</code> if <code>results</code> is empty or only
	 ...
	 *            the {@link ASTNode} of interest
	 * @return the lattice that represents the analysis state before analyzing
	 *         the node. Or <code>null</code> if the node doesn't have a
	 *         corresponding control flow node.
	 */
	protected LE getResultBefore(ICFGNode<ASTNode> node) {
		return mergeLabeledResult(getLabeledResultBefore(node),
				node.getASTNode());
	}
	...
	protected LE getResultAfter(ICFGNode<ASTNode> node) {
		return mergeLabeledResult(getLabeledResultAfter(node),
				node.getASTNode());
	}
	...
	protected LE mergeLabeledResult(IResult<LE> labeledResult, ASTNode node) {
		...
		for (ILabel label : labeledResult.keySet()) {
			if (result == null)
				result = checkNull(currentLattice
						.copy(labeledResult.get(label)));
			else
				result = checkNull(currentLattice
						.join(result, checkNull(currentLattice
								.copy(labeledResult.get(label))), node));
		}
		return result;
	}
	 ...
	 * 
	 * @param node
	 *            A node that must be inside a method.
	 */
	 ...
	 *            A node that must be inside a method.
	 * 
	 *            Nels: Caching only works if this was the last method to be
	 *            analyzed? Is there an assumption that methods are analyzed in
	 *            order an never returned to?
	 */
	...
	private void performAnalysis(MethodDeclaration methodDecl) {
		...
		AnalysisResult<LE, ASTNode, ILatticeOperations<LE>> result = worklist
				.performAnalysis();
	...
	protected WorklistTemplate<LE, ASTNode, ILatticeOperations<LE>> createWorklist(
		...
		if (transferFunction instanceof IBranchSensitiveTransferFunction)
			return factory.createBranchSensitiveWorklist(methodDecl,
					(IBranchSensitiveTransferFunction<LE>) transferFunction);
		if (transferFunction instanceof ITransferFunction)
			return factory.createBranchInsensitiveWorklist(methodDecl,
					(ITransferFunction<LE>) transferFunction);
		throw new IllegalStateException("Unknown type of transfer function: "
				+ transferFunction);
	}

	protected abstract IFlowAnalysisDefinition<LE> createTransferFunction(
			MethodDeclaration method);

	/**
	 * Returns most recently analyzed method.
	 * 
	 * @return Most recently analyzed method or <code>null</code> if no method
	 *         was analyzed yet.
	 */
	...
	/**
	 * Finds the method surrounding a given node and throws an exception if the
	 * given node is not inside a method.
	 * 
	 * @param forNode
	 * @return Method surrounding given node. This method does not return
	 *         <code>null</code> and instead throws a
