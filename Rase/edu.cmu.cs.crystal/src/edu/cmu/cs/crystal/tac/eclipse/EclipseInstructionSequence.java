...
public class EclipseInstructionSequence extends ResultfulInstruction<ASTNode> {
	...
	private final TACInstruction[] instructions; // this must be a non-empty
													// array

	public EclipseInstructionSequence(ASTNode node,
			TACInstruction[] instructions, int useAsResult,
			IEclipseVariableQuery tac) {
		super(node, tac);
		if (instructions == null || useAsResult < 0
				|| useAsResult >= instructions.length) {
			throw new IllegalArgumentException(
					"Illegal instruction sequence arguments");
		}
		if (!(instructions[useAsResult] instanceof ResultfulInstruction))
			throw new IllegalArgumentException(
					"Indicated instruction does not have result: "
							+ instructions[useAsResult]);
		this.useAsResult = useAsResult;
		this.instructions = instructions;
		// TODO sanity check: make sure result of instructions[useAsResult] is
		// not changed afterwards
	}

	public EclipseInstructionSequence(ASTNode node,
			TACInstruction[] instructions, IEclipseVariableQuery tac) {
		super(node, tac);
		...
		if (!(instructions[instructions.length - 1] instanceof ResultfulInstruction))
			throw new IllegalArgumentException(
					"Last instruction does not have result: "
	...
	protected Variable getResultVariable() {
		return ((ResultfulInstruction<?>) instructions[useAsResult])
				.getResultVariable();
	}
	...
	public TACInstruction[] getInstructions() {
		return instructions;
	}

	@Override
	public <LE> LE transfer(ITACTransferFunction<LE> tf, LE value) {
		if (AnalysisDirection.FORWARD_ANALYSIS
				.equals(tf.getAnalysisDirection())) {
	...
	public <LE> IResult<LE> transfer(
		...
		// this implementation makes the simplifying assumption that the last
		// instruction
		// in the sequence relative to the analysis direction should generate
		// results for
		// provided list of labels. All other instructions only transfer over
		// NormalLabel.
		switch (tf.getAnalysisDirection()) {
		case FORWARD_ANALYSIS:
			for (int i = 0; i < instructions.length - 1; i++) {
				IResult<LE> result = instructions[i].transfer(tf, normalOnly,
						value);
				value = result.get(normal);
			}
			return instructions[instructions.length - 1].transfer(tf, labels,
					value);
		...
		default:
			throw new UnsupportedOperationException(
					"Unknown analysis direction: " + tf.getAnalysisDirection());
		}
	}

	public <LE> LE deriveResult(ITACTransferFunction<LE> tf,
			TACInstruction targetInstr, LE value, boolean afterResult) {
		if (AnalysisDirection.FORWARD_ANALYSIS
				.equals(tf.getAnalysisDirection())) {
		...
		throw new IllegalArgumentException("Given instruction is unknown: "
				+ targetInstr);
	}

	public <LE> IResult<LE> deriveResult(
			ITACBranchSensitiveTransferFunction<LE> tf, List<ILabel> labels,
			TACInstruction targetInstr, LE value, boolean afterResult) {
		ILabel normal = NormalLabel.getNormalLabel();
		List<ILabel> normalOnly = Collections.singletonList(normal);
		...
		case FORWARD_ANALYSIS:
			...
			if (!afterResult && last == targetInstr)
				return LabeledSingleResult.createResult(value, normalOnly);
					...
					&& (last == targetInstr || this == targetInstr))
		...
		case BACKWARD_ANALYSIS:
			...
			if (afterResult && instr == targetInstr)
				return LabeledSingleResult.createResult(value, normalOnly);
					...
					&& (instr == targetInstr || this == targetInstr))
